...
window["GameExpressTip"] = GameExpressTip;
...
var RoleHeadImage = (function (_super) {
    ...
    function RoleHeadImage(url, maskRes, headImgWidth, headImgHeight) {
        ...
        debugger;
        if (_this.imgUrl.indexOf("base64") != -1) {
            that.getTexture(url, function (texture) {
                that.setBitmap(texture);
            });
        }
        else if (_this.imgUrl.indexOf("https://") == -1 && _this.imgUrl.indexOf("http://") == -1) {
 ...
 * base64转为图片
 */
    RoleHeadImage.prototype.getTexture = function (base64, callback) {
        var img = new Image();
        img["avaliable"] = true;
        img.src = base64;
        img.onload = function () {
...
window["RoleHeadImage"] = RoleHeadImage;
...
window["QIPaoCartoon"] = QIPaoCartoon;
...
var RoomVO = (function () {
    ...
    Object.defineProperty(RoomVO.prototype, "selfIsMaster", {
        get: function () {
            ...
            //     return true;
            // }
            // console.log('selfIsMaster',App.CurrChatId)
